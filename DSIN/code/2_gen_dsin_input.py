...
if __name__ == "__main__":
    ...
    # session序列，总共有SESS_COUNT个session；每个session里有DSIN_SESS_MAX_LEN点击物料
    # 'cate_id', 'brand'分别独立的做一套。
    for i in tqdm(range(SESS_COUNT)):
        ...
        for feat in sess_feature:
            ...
            sparse_feature_list.append(VarLenSparseFeat(SparseFeat(sess_name + '_' + feat, vocabulary_size=data[feat].max() + ID_OFFSET, embedding_name='feat'), maxlen=DSIN_SESS_MAX_LEN))
