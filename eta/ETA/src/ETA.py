...
class ETA(BaseModel):
    ...
    def forward(self, inputs):
        ...
        for idx, (target_field, sequence_field) in enumerate(zip(self.short_target_field, 
            ...
            target_emb = self.concat_embedding(target_field, feature_emb_dict) # [B, 4]
        ...
        for idx, (target_field, sequence_field) in enumerate(zip(self.long_target_field, 
            ...
            target_emb = self.concat_embedding(target_field, feature_emb_dict) # [B, 4], 4 is embed_size
            sequence_emb = self.concat_embedding(sequence_field, feature_emb_dict) # [B, 5, 4], 5 is seq len and 4 is embed size.
            seq_field = list(flatten([sequence_field]))[0] # flatten nested list to pick the first field
            mask = X[seq_field].long() != 0 # padding_idx = 0 required in input data. mask的地方是False，不mask的地方是True
    ...
    def topk_retrieval(self, random_rotations, target_item, history_sequence, mask, topk=5):
        ...
        hash_sim = -torch.abs(sequence_hash - target_hash).sum(dim=-1) # hamming distance。加了负号。
        hash_sim = hash_sim.masked_fill_(mask.float() == 0, -self.hash_bits) # mask的地方，用-hash_bits（最大的hamming距离）填充，

        topk_index = hash_sim.topk(topk, dim=1, largest=True, sorted=True)[1] # return (values, indices), so index 1 as top_index
        tmp = topk_index.unsqueeze(-1).expand(-1, -1, history_sequence.shape[-1])
        topk_emb = torch.gather(history_sequence, 1,
                                topk_index.unsqueeze(-1).expand(-1, -1, history_sequence.shape[-1])) # 【B， topk，embedsize】，就是把序列里的某些id的embed vec选出来
        topk_mask = torch.gather(mask, 1, topk_index) #把对应位置的mask（是否mask）也选出来
    ...
    def lsh_hash(self, vecs, random_rotations):
        ...
        """
        tmp = random_rotations.sum(axis=-1)
        rotated_vecs = torch.matmul(vecs, random_rotations) # B x seq_len x num_hashes
        hash_code = torch.relu(torch.sign(rotated_vecs)) # sign: 2-->1, -2-->-1, 0-->0; after relu, 2-->1;-1\0-->0
        return hash_code