...
if __name__ == "__main__":
    ...
    # train_g: DGLGraph
    # test_g: list of TestEdge
    train_g, test_g = utils.split_train_test_graph(g)

    # sku_info_encoder. dict. keys:["brand", "shop", "cate"]
    # sku_info_decoder. dict. keys:["brand", "shop", "cate"]
    # sku_info: dict.
    sku_info_encoder, sku_info_decoder, sku_info = utils.encode_sku_fields(
        args.item_info_data, sku_encoder, sku_decoder
    )

    num_skus = len(sku_encoder) #33344
    num_brands = len(sku_info_encoder["brand"]) # 3662
    num_shops = len(sku_info_encoder["shop"]) # 4785
    num_cates = len(sku_info_encoder["cate"]) # 79
